...
alertStream <- function(session) {
  ...
  newLines <- reactive({
    ...
    #   jsonlite::flatten(as.data.frame(jsonlite::parse_json(json)))
    # }
    ...
    # future(
    {
      ...
      print(paste("new_lines:",nrow(new_rows)))
      new_rows
    }
    # )
  })
  ...
  reactive({
    ...
    tryCatch(
      {
        lapply(new_lines,length)>0
        flow <- new_lines[unlist(lapply(new_lines,function(new_line){
          new_line$event_type == 'flow'
        }))] %>% 
