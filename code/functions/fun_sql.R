...
get_redis_list <- function(host = redis_host, 
  ...
  if(!exists("r")){
    print(paste('Creating new connection to Redist host:',host ))
    r <- redux::hiredis(host = host)
    r$PING()
  }
  
  index_last_loaded <- as.integer(index_last_loaded)
  index_end <- r$LLEN(key=key)
  
  print(paste("last load index_end:", index_last_loaded, "current index_end:",index_end ))
  ...
  print(paste("load index_start:", index_load_start, "load index_end:", index_load_end,  "current index_end:",index_end ))
  ...
  if (redux::redis_available(host = host) ||  index_end > 0 || new_entries > 0){
    tryCatch(
      {
        {
          new_json <- r$LRANGE(key=key,as.integer(index_load_start),as.integer(index_load_end)) 
          
          new_lines <- fromJSON(paste('[', paste(new_json,collapse = ','),']')) %>%
            mutate(timestamp = as_datetime(timestamp)) %>%
            mutate(timestamp_num = as.numeric(timestamp)) %>%
            filter(timestamp_num > timestamp_last_loaded)
  ...
  if(! is.null(new_lines) && nrow(new_lines) > 0){
    max_timestamp_num <- max(new_lines$timestamp_num)
    if(is.numeric(max_timestamp_num))
      timestamp_last_loaded <- max(new_lines$timestamp_num)
    print(paste("new_lines:",nrow(new_lines)))
  }else{
    print(paste("no new_lines"))
  }
...
load_history_from_rds <- function(path = history_folder,hours_to_load = 9){
  ...
  print(paste('files: ', nrow(history_files),'; duration: ',round(load_duration,2), 's; time per file: ', round(load_duration / nrow(history_files),2), 's', sep = ''))
